...
func (g *gcloudService) SetupResources(manager model.NotificationManager, config model.Config) (model.Resources, error) {
	...
	builder, err := NewBuilder(g.ctx, g.options, g.projectId, g.location, g.zone, serviceAccount, *g.pipeline.TerraformCache.Value, config.EnableOpenTofu)
	if err != nil {
...
func (g *gcloudService) GetResources() (model.Resources, error) {
	...
	builder, err := NewBuilder(g.ctx, g.options, g.projectId, g.location, g.zone, "", true, true)
	if err != nil {
		return nil, fmt.Errorf("failed to create builder: %s", err)
	}
